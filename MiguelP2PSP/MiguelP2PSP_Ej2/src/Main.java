...
public class Main {
	...
	public static void main(String[] args) {
		...
		List<ClassB_plus> listaCB=new ArrayList<ClassB_plus>();		
		...
		try {
			do {
				...
				case "2":	
					...
					ClassA_plus classA_plus=new ClassA_plus(counterAI);
					
					for(int i=0;i<numHilos;i++) {					
						ClassB_plus classB_lista=new ClassB_plus(classA_plus);
						listaCB.add(classB_lista);
					}
					for(ClassB_plus cb : listaCB) {
						cb.setLista(listaCB);
					}
					for(ClassB_plus cb : listaCB) {
						hilo=new Thread(cb);
